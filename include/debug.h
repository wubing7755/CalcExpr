 ...
 *    DEBUG_PARSER("token_t: %s", token_name(type));
 ...
 * @param ...     格式参数
 *
 * 示例：DEBUG_LOG(DEBUG_LEVEL_DEBUG, DEBUG_MODULE_PARSER, "token_t: %s", name);
 */
#define DEBUG_LOG(level, module, fmt, ...)                                                         \
    do {                                                                                           \
        if (DEBUGunlikely(g_debug_level >= (level) && ((module) & g_debug_modules))) {             \
            debug_log((level), (module), __FILE__, __LINE__, __func__, fmt, ##__VA_ARGS__);        \
        }                                                                                          \
    } while (0)
...
#define DEBUG_TRACE_ENTER(module)                                                                  \
    do {                                                                                           \
        if (DEBUGunlikely(g_debug_level >= DEBUG_LEVEL_DEBUG && ((module) & g_debug_modules))) {   \
 ...
 */
#define DEBUG_ERROR(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_ERROR, DEBUG_MODULE_ALL, fmt, ##__VA_ARGS__)

/**
 * @brief 警告信息（黄色）
 * @note 级别：DEBUG_LEVEL_WARN
 */
#define DEBUG_WARN(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_WARN, DEBUG_MODULE_ALL, fmt, ##__VA_ARGS__)

/**
 * @brief 一般信息（绿色）
 * @note 级别：DEBUG_LEVEL_INFO
 */
#define DEBUG_INFO(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_INFO, DEBUG_MODULE_ALL, fmt, ##__VA_ARGS__)

/**
 * @brief 调试信息（蓝色）
 * @note 级别：DEBUG_LEVEL_DEBUG
 */
#define DEBUG_DEBUG(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_DEBUG, DEBUG_MODULE_ALL, fmt, ##__VA_ARGS__)

/**
 * @brief 跟踪信息（青色）
 * @note 级别：DEBUG_LEVEL_TRACE
 */
#define DEBUG_TRACE(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_TRACE, DEBUG_MODULE_ALL, fmt, ##__VA_ARGS__)
 ...
 */
#define DEBUG_LEXER(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_TRACE, DEBUG_MODULE_LEXER, fmt, ##__VA_ARGS__)

/**
 * @brief parser_t 模块调试
 * @note 默认级别：DEBUG_LEVEL_INFO
 */
#define DEBUG_PARSER(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_INFO, DEBUG_MODULE_PARSER, fmt, ##__VA_ARGS__)

/**
 * @brief Calc 模块调试
 * @note 默认级别：DEBUG_LEVEL_DEBUG
 */
#define DEBUG_CALC(fmt, ...) DEBUG_LOG(DEBUG_LEVEL_DEBUG, DEBUG_MODULE_CALC, fmt, ##__VA_ARGS__)
...
static inline void debug_init(int level, unsigned module) {
    (void)level;
    (void)module;
}
static inline bool debug_parse_args(int argc, char *argv[]) {
    (void)argc;
    (void)argv;
    return false;
}
static inline void debug_set_level(int level) {
    (void)level;
}
static inline int debug_get_level(void) {
    return 0;
}
static inline void debug_set_modules(unsigned module) {
    (void)module;
}
static inline unsigned debug_get_modules(void) {
    return 0;
}
static inline void debug_enable_color(bool enable) {
    (void)enable;
}
static inline bool debug_is_color_enabled(void) {
    return false;
}
static inline void debug_set_output(const FILE *fp) {
    (void)fp;
}
