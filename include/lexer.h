 ...
 * ## 模块介绍
 *
 * 词法分析器（lexer_t）负责将输入字符串分解为词素（token_t）。
 ...
 * ## 设计要点
 *
 * - 使用"预读"模式：每次调用 lexer_next_token() 获取下一个 token
...
typedef struct {
    ...
    size_t length;     /**< 字符串长度 */
    size_t pos;        /**< 当前读取位置 */
    token_t current;   /**< 当前 token_t */
    calc_error_t err;  /**< 错误状态 */
    size_t err_pos;    /**< 错误位置 */
} lexer_t;
